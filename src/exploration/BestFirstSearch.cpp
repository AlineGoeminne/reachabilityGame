...
Path bestFirstSearch(const ReachabilityGame& game, const heuristicSignature& heuristic, Long allowedTime) {
    ...
    while(std::chrono::system_clock::now().time_since_epoch().count() - start < allowedTime) {
        ...
        if (node->state.notVisitedPlayers.size() == 0) {
            ...
            if (node->path.isANashEquilibrium(node->state.notVisitedPlayers).first) {
                // On a un EN
                return node->path;
            }
        }
                ...
                if (succ->isTarget()) {
                    ...
                    if (newReached.size() != 0) {
                        ...
                        for (unsigned int p : newReached) {
                            allPlayers.erase(p);
                            if (!newPath.isANashEquilibrium(allPlayers).first) {
                                nash = false;
                            }
                            allPlayers.insert(p);
                        }
                        ...
                        if (nash) {
                            ...
                            newNode->pathCost = heuristic(newNode, epsilon, succ, costsMap);
